...
void split_string(char *string, char substrings[][MAX_WORD_LEN], int *count) {
    char *token = strtok(string, " "); // get the first word from the string
    ...
    while (token != NULL) { // while there are more words in the string
        ...
        token = strtok(NULL, " "); // get the next word from the string
    }
...
int find_frequency(char *string, char words[][MAX_WORD_LEN], int count, int len) {
    ...
    for (int i = 0; i < count; i++) { // loop through the array of words
        ...
        for (int j = 0; j < sub_count; j++) { // loop through the array of substrings
            ...
            int len = strlen(substring); // get the length of the substring
            if (strncmp(word, substring, len) != 0) { // if the substring does not match at this position
                match = 0; // set the flag to false
                break; // break out of the inner loop
            }
            word += len + 1; // move to the next position after skipping a space
        }
...
void write_output(char *output_filename, char *main_filename, char *other_filenames[], char *substrings[], int freqs[]) { 
...
int main() {
    ...
    int main_count = extract_words(main_filename, main_words); 
    ...
    for (int i = 0; i < NUM_FILES; i++) {
        ...
        int other_count = extract_words(other_filenames[i], other_words); 
        ...
        if (substring != NULL) { // if there is a longest common substring with this file 
            fprintf(fp, "Longest common substring: %s\n", substring); // write the longest common substring to the output file 
            int freq = find_frequency(substring, other_words, other_count, len); // find the frequency of the longest common substring
            fprintf(fp, "Frequency: %d\n", freq); // write the frequency of the longest common substring to the output file 
